...
# .render_trace ผ่านคอลัมน์ "รายการตรวจ") เป็นภาษาไทย พร้อมระบุหน่วยฐานของแต่ละเกณฑ์ (ต่อ 100
# กรัม/มล., ต่อหน่วยบริโภค, ต่อหนึ่งบรรจุภัณฑ์ ฯลฯ) — ครอบคลุมทุกชื่อ "name" ที่ trace.append()
# เคยสร้างใน rules.py (ยืนยันด้วย grep ว่าไม่มีที่อื่นสร้าง trace, และยืนยันด้วยการดึงชื่อจริงทุกตัว
# จาก HC_Dashboard_Data_Public.xlsx ทั้ง 2,194 แถวว่าตรงกันครบ). ศัพท์สารอาหารหลักตรงกับ V100_TH
# ใน export_public.py (พลังงาน/น้ำตาล/โซเดียม/ไขมัน/ไขมันอิ่มตัว). หมายเหตุหน่วยฐาน:
# - "/100g"/"/100ml": rules.py แยก suffix ตาม basis จริงของ subgroup นั้น (per_100g vs per_100ml
#   ใช้หน่วยต่างกันจริง ไม่ใช่การเดา)
# - "/serving" (ต่อหน่วยบริโภค) กับ "/whole_pack" (ต่อหนึ่งบรรจุภัณฑ์) เดิมรวมกันเป็น "/pack" ตัว
#   เดียวใน rules.py ทั้งที่เป็นคนละฐานจริง (บางกฎเทียบต่อหน่วยบริโภค บางกฎคูณด้วยน้ำหนักทั้งซอง) —
#   แก้แยกใน rules.py แล้วเพื่อให้ระบุหน่วยตรงนี้ได้ถูกต้อง ไม่ใช่แค่เปลี่ยนป้ายในหน้านี้
NUTRIENT_NAME_TH = {
    "sugar/100g": "น้ำตาล (ต่อ 100 กรัม)", "sugar/100ml": "น้ำตาล (ต่อ 100 มล.)",
    "sugar/100g_prepared": "น้ำตาล (ต่ออาหารพร้อมบริโภค 100 กรัม)",
    "sodium/100g": "โซเดียม (ต่อ 100 กรัม)", "sodium/100ml": "โซเดียม (ต่อ 100 มล.)",
    "sodium/100g_prepared": "โซเดียม (ต่ออาหารพร้อมบริโภค 100 กรัม)",
    "fat/100g": "ไขมัน (ต่อ 100 กรัม)", "fat/100ml": "ไขมัน (ต่อ 100 มล.)",
    "fat/100g_prepared": "ไขมัน (ต่ออาหารพร้อมบริโภค 100 กรัม)",
    "satfat/100g": "ไขมันอิ่มตัว (ต่อ 100 กรัม)", "satfat/100ml": "ไขมันอิ่มตัว (ต่อ 100 มล.)",
    "satfat/100g_prepared": "ไขมันอิ่มตัว (ต่ออาหารพร้อมบริโภค 100 กรัม)",
    "energy/100g": "พลังงาน (ต่อ 100 กรัม)", "energy/100ml": "พลังงาน (ต่อ 100 มล.)",
    "energy/100g_prepared": "พลังงาน (ต่ออาหารพร้อมบริโภค 100 กรัม)",
    "sugar/100kcal": "น้ำตาล (ต่อพลังงาน 100 กิโลแคลอรี)",
    ...
    "fat_two_tier/tier1_max": "ไขมัน (ต่อ 100 กรัม, เกณฑ์ที่ 1)",
...
def page_criteria():
    ...
    # "ขนมหวาน/ขนมไทยทำมือ (thai_dessert)" ไม่ใช่กลุ่มย่อยที่มีอยู่จริงในประกาศฉบับที่ 4 —
    # เป็น subgroup ที่ระบบสร้างขึ้นเองเพื่อ force 3.3 ผลิตภัณฑ์สูตรไม่ชัด (ruling 2026-06-15)
    # เท่านั้น การจัดเกรดจริงยังทำงานเหมือนเดิมทุกอย่าง แค่ไม่แสดงเป็น "เกณฑ์" บนหน้านี้ เพราะ
    # หน้านี้ตั้งใจสรุปเฉพาะเกณฑ์ที่มีอยู่จริงในกฎหมาย (per user 2026-09-16)
    ref = ref[ref["Subgroup"] != "thai_dessert"]
